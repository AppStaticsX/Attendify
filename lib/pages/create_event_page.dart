...
class _CreateEventPageState extends State<CreateEventPage>
  ...
  void initState() {
    ...
    // Set default values
    creditHoursController.text = '3';
    minAttendanceController.text = '80';
  ...
  Future<void> _saveEvent() async {
    ...
    try {
      final db = context.read<EventDatabase>();
      await db.addEvent(courseName, selectedDays, professorName);
      
      // Update the newly created event with additional fields
      if (db.currentEvents.isNotEmpty) {
        final newEvent = db.currentEvents.last;
        newEvent.courseCode = courseCode;
        newEvent.creditHours = creditHours;
        newEvent.lectureTime = lectureTime;
        newEvent.location = location;
        newEvent.semester = semester;
        newEvent.minAttendanceRequired = minAttendance;
        newEvent.totalLecturesPlanned = totalLectures;
        await newEvent.save();
      }
      
      if (mounted) {
        _showCustomToast('Course created successfully!', Iconsax.tick_circle);
      ...
      if (mounted) {
        ...
        _showCustomToast('Failed to create course: $e', Iconsax.close_circle,
            durationInSeconds: 3);
      }
    }
  }
